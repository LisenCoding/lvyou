        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '路线推荐名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.luxianName" placeholder="路线推荐名称" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('luxianLiuyan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/lvyouguanlixitong/upload/luxianLiuyanMuBan.xls"
                        >批量导入路线推荐评论数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('luxianLiuyan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="luxianLiuyanUploadSuccess"
                                :on-error="luxianLiuyanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('luxianLiuyan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入路线推荐评论数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('luxianLiuyan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "luxianLiuyan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('luxianLiuyan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('luxianLiuyan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>
                            <el-button v-if="isAuth('luxianLiuyan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "luxianLiuyan/update",
                        method: "post",
                        data: {
                            id:id,
//                            luxianLiuyanTypes:luxianLiuyanTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
